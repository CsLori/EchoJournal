...
object AmplitudeNormalizer {
    ...
    fun normalize(
        ...
        require(trackWidth >= barWidth + spacing) {
            "Track width must be greater than or equal to the sum of bar width and spacing."
        }
        if (sourceAmplitudes.isEmpty()) {
            return emptyList()
        }
        val barCounts = (trackWidth / (barWidth + spacing)).roundToInt()
        val resampled = resampleAmplitudes(sourceAmplitudes, barCounts)
    ...
    private fun downSample(
        ...
        return List(targetSize) { index ->
            ...
            source.subList(start, end).average().toFloat()
        }
    }
}